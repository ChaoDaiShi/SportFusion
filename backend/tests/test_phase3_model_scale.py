...
class TestTargetLeakage(unittest.TestCase):
    ...
    def test_w1_not_in_sportshare_features(self):
        ...
        fv = build_sportshare_features("体育赛事运营，运动器材销售，餐饮服务", 8911, sport_score=0.5)
    ...
    def test_no_business_line_ratio_in_features(self):
        ...
        fv = build_sportshare_features("体育赛事，运动器材，餐饮服务", 8911, sport_score=0.5)
...
class TestPredictionIntervals(unittest.TestCase):
    ...
    def test_interval_clips_at_boundaries(self):
        lower, upper = build_prediction_interval(0.05, 0.20)
        self.assertEqual(lower, 0.0)
        lower2, upper2 = build_prediction_interval(0.95, 0.20)
...
class TestMacroCalibration(unittest.TestCase):
    ...
    def test_boundary_split_sums_to_total(self):
        allocations = [
            ScaleAllocationResult(allocated_output=10.0, is_traditional_boundary=True),
            ScaleAllocationResult(allocated_output=5.0, is_traditional_boundary=True),
            ScaleAllocationResult(allocated_output=3.0, is_traditional_boundary=False),
            ScaleAllocationResult(allocated_output=2.0, is_traditional_boundary=False),
        ]
        split = compute_boundary_split(allocations)
        self.assertAlmostEqual(split["inside_traditional_boundary_output"], 15.0, places=2)
        self.assertAlmostEqual(split["outside_traditional_boundary_output"], 5.0, places=2)
        self.assertAlmostEqual(split["inside_pct"] + split["outside_pct"], 100.0, places=2)

    def test_compute_enterprise_weight_default(self):
        w = compute_enterprise_weight(0.5, 0.7, "direct", alpha=0.20)
        expected = 0.5 + 0.20 * 0.7
...
class TestScenarioEngine(unittest.TestCase):
    ...
    def test_all_calibrations_present(self):
        configs = generate_scenario_configs()
        cals = sorted(set(c.evidence_calibration for c in configs))
